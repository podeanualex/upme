...
class UPME_Profile_Header {
    ...
    public function get_cover_image(){
        global $upme,$upme_roles,$upme_save;;
        extract($this->profile_shortcode_params);
        
        $view = isset($view) ? $view : '';

        $user_cover_pic = isset($upme_save->upme_fileds_meta_info_array['user_cover_pic']) ? $upme_save->upme_fileds_meta_info_array['user_cover_pic'] : array();
        extract($user_cover_pic);       
        ...
        $cover_image_default = isset($upme->upme_options['default_cover_image_path']) ?
            $upme->upme_options['default_cover_image_path'] : upme_url . 'img/cover-default.png';
    ...
    public function get_profile_pic_display(){
        ...
        if (isset($current_option['profile_image_popup_status'])  && $current_option['profile_image_popup_status'] == '1') {
            $profile_url = '#upme_profile_image_inner_modal';
        }
        ...
        if ($upme->get_option('clickable_profile')) {
            if ($upme->get_option('clickable_profile') == 1) {
                if ('compact' == $view) {
                    ...
                    $profile_pic_display .= '<a href="'.$profile_url.'" upme-data-user-id="'.$id.'" >' . $upme->pic($id, 50) . '</a>';
                }
            }else{
    ...
    public function profile_header_one($params){
        ...
        $display .= '<div class="upme-profile-title" >' . $this->profile_title_display ;
