...
class UPME_Social_Connect{
	...
	public function register_user($result){
		global $upme_register,$upme_email_templates,$upme_options;
		...
		if($result->status){
			...
			if(!$user){
                ...
                $upme_user_role		= isset($_GET['upme_user_role']) ? $_GET['upme_user_role'] : get_option('default_role');
                $user_roles_registration = isset($upme_options->upme_settings['choose_roles_for_registration']) ? 
                	$upme_options->upme_settings['choose_roles_for_registration'] : array() ;
                $allow_user_role_registration = isset($upme_options->upme_settings['select_user_role_in_registration']) ?
                	$upme_options->upme_settings['select_user_role_in_registration'] : 0;
        
		        $allowed_user_roles = is_array($user_roles_registration) ? $user_roles_registration : array($user_roles_registration);

        		if($upme_user_role != '' && ($upme_user_role == get_option('default_role') || ($allow_user_role_registration && in_array($upme_user_role, $allowed_user_roles)) )){
	            ...
	            $link = get_permalink($upme_settings['login_page_id']);

	            $send_params = array('username' => $username , 'password' => $user_pass, 'login_link' => $link);
                $email_status = $upme_email_templates->upme_send_emails('reg_social_user', '' , '' , '' ,$send_params,$user_id);
                $send_params = array('username' => $username , 'email' => $result->email);
                $admin_email_status = $upme_email_templates->upme_send_emails('reg_social_admin', '' , '' , '' ,$send_params,$user_id);
