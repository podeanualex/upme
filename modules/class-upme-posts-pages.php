...
class UPME_Posts_Pages{
	...
	function __construct() {
        ...
        add_action('template_redirect',array($this, 'check_post_page_type'));
	}
    
    public function check_post_page_type(){
        global $post,$upme,$upme_bbpress_topic_content,$upme_woocommerce_topic_content;

        if(isset($post->post_type) && $post->post_type == 'topic' && class_exists( 'bbPress' ) && (is_page() || is_single())){
            $upme_bbpress_topic_content = $this->content_filter('');
        }else if(isset($post->post_type) && $post->post_type == 'product' && class_exists( 'WooCommerce' ) && (is_page() || is_single())){
            $upme_woocommerce_topic_content = $this->content_filter('');            
        }
        else if(is_page() || is_single()){
            add_filter( 'the_content', array($this,'content_filter') , 20 );
            add_filter( 'the_content', array($this,'profile_after_post') , 20 );
        }
    }
    ...
    public function content_filter($content){
        global $post,$upme_roles,$upme_options,$upme;
    ...
    public function profile_after_post($content){
        global $post,$upme,$profile_after_post_status;
        $author_id = isset($post->post_author) ? $post->post_author : 0;

        if(is_single() && $upme->get_option('display_profile_after_post') == '1' && $author_id != 0 && $profile_after_post_status != 'ACTIVE'){
            $template = $upme->get_option('author_post_profile_template');
     ...
     public function profile_tab_items($display,$params){
        ...
        if( is_user_logged_in() && $this->current_user && $this->current_user == $params['id'] &&
            ...
            if($params['title_display'] == 'enabled'){            
                $display .= '<div class="upme-profile-tab-title">'. apply_filters('upme_profile_tab_items_favorites_title', __('Favorites','upme'),$params) . '</div>';
            }
        ...
        if( is_user_logged_in() && $this->current_user && $this->current_user == $params['id'] &&
            ...
            if($params['title_display'] == 'enabled'){            
                $display .= '<div class="upme-profile-tab-title">'. apply_filters('upme_profile_tab_items_reader_title', __('Reader','upme'),$params) . '</div>';
            }
        ...
        if( $this->recommend_enabled_status && $recommend_user_role_status){
            ...
            if($params['title_display'] == 'enabled'){            
                $display .= '<div class="upme-profile-tab-title">'. apply_filters('upme_profile_tab_items_recommended_title', __('Recommended','upme'),$params) . '</div>';
            }
    ...
    public function profile_view_forms($display,$params){
        ...
        if($view != 'compact' && $hide_profile_tabs != 'yes'){
            if( is_user_logged_in() && $this->current_user && $this->current_user == $params['id'] &&
                ...
                $display .= '<div class="upme-favorite-panel upme-profile-tab-panel" style="display:none;" >
            ...
            if( is_user_logged_in() && $this->current_user && $this->current_user == $params['id'] &&
                ...
                $display .= '<div class="upme-reader-panel upme-profile-tab-panel" style="display:none;" >
            ...
            if( $this->recommend_enabled_status && $recommend_user_role_status){
                ...
                $display .= '<div class="upme-post-recommend-panel upme-profile-tab-panel" style="display:none;" >
