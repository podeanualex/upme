...
jQuery(document).ready(function($) {
    ...
    /* Toggle inline edit */
    $(document).on('click','.upme-edit', function(){
...
//    });
    ...
    jQuery(".upme_field_validation_setting").hide();

    jQuery("#upme_profile_validation_field").change(function(){
        if(jQuery(this).val() != '0'){
            jQuery("#upme_profile_validation_field_loading").html(UPMEAdmin.validationLoading).show();
            jQuery(".upme_field_validation_setting").hide();
            jQuery(".upme_field_validation_setting_value").val('');

            jQuery.post(
                UPMEAdmin.AdminAjax,
                {
                    'action' : 'upme_field_validation_load_settings',
                    'field_key':  jQuery(this).val()
                },
                function(response){
                    jQuery("#upme_profile_validation_field_loading").html("").hide();

                    if(response.status == 'error'){
                        jQuery("#upme_profile_validation_field_loading").html(response.msg).show();
                        jQuery(".upme_field_validation_setting").hide();

                    }else if(response.status == 'empty'){
                        jQuery("#upme_profile_validation_field_loading").html(response.msg).show();
                        jQuery(".upme_field_validation_setting").show();


                    }else if(response.status == 'success'){
                        jQuery(".upme_field_validation_setting").show();

                        if(response.data.min_length){
                            jQuery('#upme_field_validation_min_length').val(response.data.min_length);
                        }
                        if(response.data.max_length){
                            jQuery('#upme_field_validation_max_length').val(response.data.max_length);
                        }
                    }
    ...
    jQuery("#upme_field_validation_save").click(function(){

        jQuery("#upme_profile_validation_field_loading").html(UPMEAdmin.validationSaving).show();

        var validations = {};

        jQuery(".upme_field_validation_setting").each(function(){
            validations[jQuery(this).attr('data-validation-type')] = jQuery(this).find(".upme_field_validation_setting_value").val();
        });

        jQuery.post(
            UPMEAdmin.AdminAjax,
            {
                'action' : 'upme_field_validation_save_settings',
                'field_key':  jQuery("#upme_profile_validation_field").val(),
            ...
            function(response){
                ...
                if(response.status == 'error'){
                    ...
                    jQuery("#upme_profile_validation_field_loading").html(UPMEAdmin.validationSaved).show();
    ...
    jQuery(document).on('click','.upme-clone', function(){

        var cloned_row = jQuery(this).closest('.upme-field-setting-row');
        var random_key = upme_get_random_meta_number();

        var new_field = cloned_row.clone();
        var meta_key = new_field.attr('data-field-item-meta');
        new_field.find('.upme-field-table-data-meta-key').html('<span class="upme-mobile-custom-field-title">Meta Key : </span>' + meta_key + '_' + random_key);
        new_field.attr('data-field-item-meta',meta_key + '_' + random_key);
        new_field.find('.upme-trash-action').remove();
        new_field.find('.upme-clone-action').remove();
        new_field.find('.upme-edit-action .upme-edit').attr('data-field-meta', meta_key + '_' + random_key );
        

        jQuery.post(
            UPMEAdmin.AdminAjax,
            {
                'action'       : 'upme_clone_custom_field',
                'field_meta'   :  meta_key,
                'random_key'   :  random_key
            },
            function(response){
                cloned_row.after(new_field);
            },"json"
...
function upme_get_random_meta_number(){
    return Math.floor((Math.random() * 99999) + 1111);
}