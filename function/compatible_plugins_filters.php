...
add_filter('upme_search_custom_orderby_meta_joins','upme_search_custom_orderby_meta_joins',10,2);
function upme_search_custom_orderby_meta_joins($orderby_meta_custom_joins,$search_custom_orderby_meta_joins_params){
    global $wpdb,$upme;

    if(! (isset($upme->upme_args['memberpress_active_memberships'])
        && $upme->upme_args['memberpress_active_memberships'] == 'yes')){
    ...
    $mepr_db = new MeprDb();
    $orderby_meta_custom_joins = ' inner JOIN ' . $mepr_db->members . ' as meprtr ON (users.ID = meprtr.user_id) ';
    return $orderby_meta_custom_joins;
}

add_filter('upme_search_custom_orderby_meta_where','upme_search_custom_orderby_meta_where',10,2);
function upme_search_custom_orderby_meta_where($upme_search_custom_orderby_meta_where,$search_custom_orderby_meta_joins_params){
    if(! (isset($upme->upme_args['memberpress_active_memberships'])
        && $upme->upme_args['memberpress_active_memberships'] == 'yes')){
    ...
    array_push($upme_search_custom_orderby_meta_where, " (meprtr.active_txn_count > 0 ) ");       
    return $upme_search_custom_orderby_meta_where;
}